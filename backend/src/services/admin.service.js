...
import { VideoReport } from '../models/VideoReport.js';
...
export async function getDashboardStats() {
  try {
    // Get data from Models (all in parallel)
    const [
      userStats,
      videoStats,
      views24h,
      ...
      User.getStatistics(),
      Video.getStatistics(),
      ViewHistory.getViews24h(),
      UserReport.countPending(),
      VideoReport.countPending(),
      Appeal.countPending(),
      SystemSettings.getMaintenanceMode(),
      SystemSettings.getServiceMaintenanceMode()
    ]);
...
export async function getTopVideos(limit = 5) {
  try {
    return await Video.getTopByViews(limit);
  } catch (error) {
...
export async function getPendingReports(limit = 5) {
  try {
    const [userReports, videoReports] = await Promise.all([
      UserReport.getPending(limit),
      VideoReport.getPending(limit)
    ]);
...
export async function getPendingAppeals(limit = 5) {
  try {
    return await Appeal.getPending(limit);
  } catch (error) {
...
export async function getAllUsers(options = {}) {
  try {
    ...
    const [users, total] = await Promise.all([
      User.getAllUsers({ limit, offset, role, banned, search }),
      User.getTotalCount({ role, banned, search })
    ]);
...
export async function getStaffMembers(options = {}) {
  try {
    return await User.getStaffMembers(options);
  } catch (error) {
...
export async function promoteToStaff(username, performedById) {
  try {
    const user = await User.promoteToStaff(username);
...
export async function demoteStaff(username, performedById) {
  try {
    const user = await User.demoteStaff(username);
...
export async function deleteStaffAccount(username, performedById) {
  try {
    ...
    const staffList = await User.getStaffMembers({ isDemoted: true });
    const user = staffList.find(s => s.username === username);
    if (!user || user.role !== 'staff' || !user.isDemoted) {
      throw new ApiError(400, 'Can only delete demoted staff accounts', 'CANNOT_DELETE');
    }
    
    await User.deleteStaffAccount(username);
...
export async function banUser(username, reason, durationDays, performedById) {
  try {
    const user = await User.banUser(username, reason, durationDays);
...
export async function unbanUser(username, performedById) {
  try {
    ...
    const allUsers = await User.getAllUsers({ limit: 1000 });
    const user = allUsers.find(u => u.username === username);
    if (!user) {
      throw new ApiError(404, 'User not found', 'USER_NOT_FOUND');
    }
    
    const updatedUser = await User.unbanUser(username);
