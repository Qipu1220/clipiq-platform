...
export function AdminDashboard({ onVideoClick, onViewUserProfile }: AdminDashboardProps) {
  ...
  // Calculate stats from API data or fallback to Redux state
  const totalUsers = dashboardData?.stats.users.total ?? allUsers.filter(u => u.role === 'user').length;
  const totalStaff = dashboardData?.stats.users.staff ?? allUsers.filter(u => u.role === 'staff').length;
  const totalVideos = dashboardData?.stats.videos.total ?? videos.length;
  const bannedUsers = dashboardData?.stats.users.banned ?? allUsers.filter(u => u.banned).length;
  const activeReports = dashboardData?.stats.reports.pending ?? [...videoReports, ...userReports].filter(r => r.status === 'pending').length;
  const todayVideos = dashboardData?.stats.videos.uploadedToday ?? videos.filter(v => {
    const videoDate = v.uploadDate ? new Date(v.uploadDate) : (v.createdAt ? new Date(v.createdAt) : null);
    return videoDate && videoDate.toDateString() === new Date().toDateString();
  }).length;
  ...
  const handleBanUser = (username: string) => {
    setConfirmAction({
      ...
      message: `Bạn có chắc muốn ban người dùng ${username}?`,
      confirmText: 'Ban',
      confirmColor: '#ff3b5c',
      onConfirm: async () => {
        try {
          await banUserApi(username, 'Banned by admin'); // Permanent ban
          dispatch(banUserByUsername({ username, reason: 'Banned by admin' }));
          toast.success(`Đã ban người dùng ${username}`);
          setShowConfirmModal(false);
          fetchUsers();
          if (activeTab === 'system-logs') {
            fetchSystemLogs();
          ...
          toast.error(error.response?.data?.error || 'Không thể ban người dùng');
        }
      }
    });
    setShowConfirmModal(true);
              ...
              <div className="space-y-6">
                ...
                {usersLoading ? (
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {users.map(user => (
                      <Card key={user.id} className="bg-zinc-950/50 border-zinc-900/50 rounded-xl overflow-hidden">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4 flex-1">
                              ...
                              <div className="text-right">
                                ...
                                <div className="text-white font-medium">{user.videoCount}</div>
                              </div>
                            </div>
