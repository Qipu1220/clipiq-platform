...
import { updateDisplayName, updateAvatar, logout } from '../../store/authSlice';
...
export function UserProfile({ onVideoClick, onNavigateHome, onNavigateUpload }: UserProfileProps) {
  ...
  const myVideos = videos.filter(v => v.uploaderUsername === currentUser?.username);
  const likedVideos = videos.filter(v => v.likes.includes(currentUser?.username || ''));
  
  // Get saved videos from "Đã lưu" playlist
  const savedPlaylist = playlists.find(
    p => p.username === currentUser?.username && p.name === 'Đã lưu'
  );
  const savedVideos = savedPlaylist 
    ? videos.filter(v => savedPlaylist.videoIds.includes(v.id))
    : [];
  ...
  const totalLikes = myVideos.reduce((sum, video) => sum + video.likes.length, 0);

  // Sort videos
  const sortVideos = (vids: typeof videos) => {
    const sorted = [...vids];
    if (sortBy === 'newest') {
